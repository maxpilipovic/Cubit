...
TEST_CASE("A ray through empty space hits nothing")
{
    ...
    const VoxelRayHit hit =
        VoxelRaycast::Cast(chunk, CentreOf(8, 8, 0), glm::vec3(0, 0, 1), 32.0f);
...
{
    Chunk chunk;
    chunk.SetBlock(8, 8, 5, BlockType::Solid);
...
TEST_CASE("The hit normal points back along the ray")
{
    ...
    {
        const VoxelRayHit hit =
            VoxelRaycast::Cast(chunk, CentreOf(8, 8, 3), glm::vec3(0, 0, 1), 32.0f);
        REQUIRE(hit.Hit);
    ...
    {
        const VoxelRayHit hit =
            VoxelRaycast::Cast(chunk, CentreOf(8, 8, 13), glm::vec3(0, 0, -1), 32.0f);
        REQUIRE(hit.Hit);
        CHECK(hit.Normal == glm::ivec3(0, 0, 1));
    }
    ...
    {
        const VoxelRayHit hit =
            VoxelRaycast::Cast(chunk, CentreOf(3, 8, 8), glm::vec3(1, 0, 0), 32.0f);
        REQUIRE(hit.Hit);
    ...
    {
        const VoxelRayHit hit =
            VoxelRaycast::Cast(chunk, CentreOf(13, 8, 8), glm::vec3(-1, 0, 0), 32.0f);
        REQUIRE(hit.Hit);
        CHECK(hit.Normal == glm::ivec3(1, 0, 0));
    }
    ...
    {
        const VoxelRayHit hit =
            VoxelRaycast::Cast(chunk, CentreOf(8, 3, 8), glm::vec3(0, 1, 0), 32.0f);
        REQUIRE(hit.Hit);
    ...
    {
        const VoxelRayHit hit =
            VoxelRaycast::Cast(chunk, CentreOf(8, 13, 8), glm::vec3(0, -1, 0), 32.0f);
        REQUIRE(hit.Hit);
        CHECK(hit.Normal == glm::ivec3(0, 1, 0));
    }
}
...
TEST_CASE("The block next to the hit face is always empty")
{
    ...
    const VoxelRayHit hit =
        VoxelRaycast::Cast(chunk, CentreOf(8, 8, 2), glm::vec3(0, 0, 1), 32.0f);

    REQUIRE(hit.Hit);
    const glm::ivec3 placement = hit.Block + hit.Normal;

    CHECK(placement == glm::ivec3(8, 8, 7));
    CHECK_FALSE(chunk.IsBlockSolid(placement.x, placement.y, placement.z));
}

TEST_CASE("A ray does not reach past its maximum distance")
{
    Chunk chunk;
    chunk.SetBlock(8, 8, 12, BlockType::Solid);

    const glm::vec3 origin = CentreOf(8, 8, 2);

    const VoxelRayHit tooShort =
        VoxelRaycast::Cast(chunk, origin, glm::vec3(0, 0, 1), 4.0f);
    CHECK_FALSE(tooShort.Hit);

    const VoxelRayHit longEnough =
        VoxelRaycast::Cast(chunk, origin, glm::vec3(0, 0, 1), 32.0f);
...
{
    Chunk chunk;
    chunk.SetBlock(8, 8, 10, BlockType::Solid);
...
TEST_CASE("A ray starting inside a solid block reports that block")
{
    ...
    const VoxelRayHit hit =
        VoxelRaycast::Cast(chunk, CentreOf(8, 8, 8), glm::vec3(0, 0, 1), 32.0f);
...
TEST_CASE("An unnormalized direction gives the same result")
{
    ...
    const VoxelRayHit scaled =
        VoxelRaycast::Cast(chunk, origin, glm::vec3(0, 0, 7.3f), 32.0f);
...
TEST_CASE("A zero direction cannot hit anything")
{
    ...
    const VoxelRayHit hit =
        VoxelRaycast::Cast(chunk, CentreOf(8, 8, 2), glm::vec3(0, 0, 0), 32.0f);
...
TEST_CASE("A diagonal ray enters through the nearer face")
{
    ...
    Chunk chunk;
    chunk.SetBlock(9, 9, 8, BlockType::Solid);
...
TEST_CASE("A ray from outside the chunk reaches blocks inside it")
{
    Chunk chunk;
    chunk.SetBlock(0, 8, 8, BlockType::Solid);
...
TEST_CASE("Rays never report a block they did not enter")
{
    ...
    for (int i = 0; i < 64; ++i)
    {
        ...
        const VoxelRayHit hit =
            VoxelRaycast::Cast(chunk, origin, direction, 64.0f);

        if (!hit.Hit)
            continue;

        REQUIRE(chunk.IsBlockSolid(hit.Block.x, hit.Block.y, hit.Block.z));

        const glm::ivec3 placement = hit.Block + hit.Normal;
        REQUIRE_FALSE(chunk.IsBlockSolid(placement.x, placement.y, placement.z));
    }
}
