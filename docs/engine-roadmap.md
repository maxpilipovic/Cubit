...
- [x] **B2. Terrain collapse** — blocks left with no support fall. **In scope by the user's
  ...
  **Left out on purpose:** falling blocks doing damage (a game rule, D-level) and any visible
  ...
  relight cost.
- [x] **B3. A way to draw anything that is not a chunk.** **Done 2026-09-21**, in six
  tasks recorded in `docs/superpowers/plans/2026-09-18-non-chunk-geometry.md`.
  - `VoxelFaces::All[6]` is now the one definition of a block face — normal, corners
    and shading — that `ChunkMesher` used to own alone, plus a shared
    `ShadeVertex(color, faceShade, ao, light)`. Both meshers read one definition of a
    face rather than each keeping a copy.
  - `ModelMesher::Build(const VoxModel&) -> MeshGeometry` meshes a standalone `.vox`
    model against that same table, the same ambient occlusion and the same
    diagonal-split rule — a sibling of `ChunkMesher`, not a caller of it.
  - `Mesh` wraps live GL buffers for one upload, and `WorldScene::DrawMesh(mesh,
    transform, brightness)` draws it through the same shader as a chunk, with
    `brightness` multiplying in before the fog mix. It is only valid inside or after a
    `Render` in the same frame, because it reuses that frame's view-projection; a debug
    assert enforces this.
  - `game/assets/models/player.vox` is a placeholder, generated by a committed script
    and copied next to the exe by premake. `DrawRemotePlayers`
    (`game/GameApp/src/GameApp.cpp`) loads and meshes it once at layer construction,
    throwing if it is missing, then scales it from its own height to whatever the
    simulation says a player's height is, stands it feet-down and centred on the
    collision box, turns it by `-pose.Yaw`, and lights it with one sample of the
    world's sky light where it stands, floored at `ChunkMesher::LightFloor`.

  Team colours and a first-person held tool, the other two things this item named, are
  ...
  **Tests:** `ModelMesher` has five engine tests, and the placeholder asset has one
  ...
  no model loading; remote players are `DebugDraw` wireframe boxes (`DrawRemotePlayers`,
  `Sandbox.cpp:570`). Player models, a held tool and team colours all need it.
- [ ] **B3a. Team colours.** B3 named this and did not deliver it. `ModelMesher` bakes
  each model's own palette colours into its vertices; there is no way to tint or swap
  a palette per team, and nothing chooses one.
